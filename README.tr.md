...
Adapter’lar genel yeteneklerini protokol v1 üzerinden bildirir. `containers.logs` gibi adlar yerleşik entegrasyon değil, yetenek örnekleridir. İsteğe bağlı [Docker adapter’ı](docs/docker-adapter.md), ayrı bir Python executable olarak gerçek container verisi, onaylı yaşam döngüsü aksiyonları ve belirli bir container’a bağlı metin shell’i sunar. İncelediğiniz kaynak koddan ayrıca kurulmalıdır; dört native binary paketi alana özgü provider kurmaz. Git, PostgreSQL, Kubernetes, süreç, port veya veritabanı adapter’ı pakete dahil değildir. Sekizinci bölümdeki Capability Browser, canlı controller tanılarını opak yetenek sözleşmesine göre gruplar ve her sözleşmeyi bildiren adapter’ları listeler; yetenek çağırmaz veya verilerini tüketmez.
