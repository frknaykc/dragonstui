...
The optional showcase derives its M53–M58 Log Viewer, metric graph, heatmap, status matrix, Timeline, and Error/Stack Trace views exclusively from those explicit observation variants. These are render-time projections of the existing bounded 16-entry live history, not host-side telemetry stores. The Error view groups by explicit signature or message fallback; any count and first/last values therefore describe only retained source events. The adapter host still has no adapter-specific observability routing and no capability-action invocation surface.
